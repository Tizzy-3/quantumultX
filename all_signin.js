...
async function hashiqiSignin() {
  const cookie = $prefs.valueForKey('hashiqi_cookie');
...
async function imyaiSignin() {
  const jwt = $prefs.valueForKey('imyai_jwt');
...
async function kingdeeSignin() {
  const cookie = $prefs.valueForKey('kingdee_cookie');
  const csrf = $prefs.valueForKey('kingdee_csrf') || '';
