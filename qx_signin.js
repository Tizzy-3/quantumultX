...
main()
  .then(async () => {
    const report = buildMarkdownReport(state.lines);
    await pushWeCom(report);
    notify("Daily Sign-in", "Done", state.lines.map((x) => `${x.name}: ${x.status}`).join("\n"));
    done();
  })
  .catch(async (error) => {
    const message = error && error.stack ? error.stack : String(error);
    state.lines.push({ name: "Runner", status: "Failed", detail: message });
    await pushWeCom(buildMarkdownReport(state.lines));
    notify("Daily Sign-in", "Failed", message.slice(0, 800));
    done();
  });
...
async function runStep(name, fn) {
  try {
    ...
    state.lines.push({ name, status: result.status || "OK", detail: result.detail || "" });
  } catch (error) {
    state.lines.push({
      name,
      status: "Failed",
...
async function signHashiqi() {
  ...
  if (savedCookie) {
    ...
    return skipped("missing HASHIQI_COOKIE or HASHIQI_USERNAME/HASHIQI_PASSWORD");
  }
  ...
  return {
    status: signed ? (signedBefore ? "Already signed" : "Signed") : "Unknown",
    detail: `+${reward} dog food; total ${total || "unknown"}`,
  };
}
...
async function signImyai() {
  ...
  if (!jwt) {
    return skipped("missing IMYAI_JWT");
  }
  ...
  return {
    status: signStatus,
    detail: `basic ${valueOf(balance.model3Count)}; advanced ${valueOf(balance.model4Count)}; draw ${valueOf(balance.drawMjCount)}`,
  };
}
...
async function signKingdee() {
  ...
  if (!cookie) {
    return skipped("missing KINGDEE_COOKIE");
  }
  ...
  return {
    status,
    detail: `+${todayCoins} coins; lottery ${lottery}; total ${coins}; streak ${days}`,
  };
}
...
function buildMarkdownReport(lines) {
  const date = todayString();
  const body = lines.map((item) => {
    const detail = item.detail ? `\n> ${item.detail}` : "";
    return `## ${item.name}\n> Status: **${item.status}**${detail}`;
  }).join("\n\n");
  return `# Daily Sign-in Report ${date}\n\n${body}\n\n> Automated by Quantumult X`;
}
